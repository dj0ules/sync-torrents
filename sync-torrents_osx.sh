...
# Log file
_logfile=$HOME/Downloads/push-torrent.log
_tmp_log=$HOME/Downloads/push-torrents.log.tmp
...
_log_title="Push Torrents - $_sync_date"
...
if [ "$?" -eq 0 ]; then
	if [ "$_torrent_count" -gt 0 ]; then
		...
		find $_tvshow_dir -type d -depth 2 -delete
	else
		exit 0
	fi
else
